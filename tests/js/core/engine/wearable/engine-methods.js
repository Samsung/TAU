...
(function (document, console, ns) {
	...
	var engine = ns.engine,
		...
		WidgetButton = ns.widget.core && ns.widget.core.Button,
		testElement;
	...
	module("core/engine", {
		setup: function () {
			testElement = document.getElementById("button1");
			engine.run();
			engine.instanceWidget(testElement, 'Button');
		},
		teardown: function () {
	...
	test("Checking .getBinding method - one parameter", function () {
		...
		ok(tempBinding instanceof WidgetButton, "Widget is a instanceof Button");

		tempBinding2 = engine.getBinding("button1");
	...
	test("Checking .getBinding method - two parameters", function () {
		...
		tempBinding = engine.getBinding(testElement, "Button");
		ok(!!tempBinding, "Widget reference was returned");
		ok(tempBinding instanceof BaseWidget, "Widget is a instanceof BaseWidget");
		ok(tempBinding instanceof WidgetButton, "Widget is a instanceof Button");

		tempBinding2 = engine.getBinding("button1", "Button");
		ok(tempBinding === tempBinding2, "Passing HTMLElement reference and string ID returns that same object reference");
	});
